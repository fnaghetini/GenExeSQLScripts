...
class Log:
    ...
    def display_date(self):
        execution_date = f'Date: {self.execution_date}\n'
        self.logfile.write(execution_date)
        self.logfile.flush()

    def show_error_message(self, error_message):
        full_error_message = f'ERROR ({datetime.now().strftime("%H:%M:%S")}): {error_message}!\n'
        self.logfile.write(full_error_message)
        self.logfile.flush()
        messagebox.showerror('Error', full_error_message)
    ...
    def display_execution_time(self, new_start_time=None, full_exec_time=False):
        if new_start_time is not None:
            ...
            self.logfile.write(f'Execution time: {execution_time}\n')
            self.logfile.flush()
        else:
    ...
    def show_completion_message(self, completion_message, write_log=True, show_messagebox=True):
        if write_log:
            full_completion_message = f'\n\nPROCESS FINISHED! {completion_message}'
            self.logfile.write(full_completion_message)
            self.logfile.flush()
        if show_messagebox:
            messagebox.showinfo('Process Finished', completion_message)
